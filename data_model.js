...
module.exports = {
    ...
    signIn: function(email, password) {
        return new Promise(async (resolve, reject) => {
            ...
            db.serialize(() => {
                db.run('SELECT user_id, encrypted_password FROM users WHERE email = ?', [email], (err, rows) => {
                    console.log(rows)
                    if (err) {
                        reject(err)
                    } else {
                        bcrypt.compare(password, rows.encrypted_password).then(result => {
                            if (result) {
...
//
    ...
    addFollow: function(userId, followId) { 
        return new Promise(function (resolve, reject) {
            const db = new sqlite3.Database('./goaltracker.db');
            db.serialize(() => {
                db.run('INSERT INTO follows (user_id, goal_id) VALUES (?,?)',[userId,followId], (err) => {
                    if (err) {
                        reject(err);
                    } else { 
                        resolve();
